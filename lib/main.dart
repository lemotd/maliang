  ...
  void _showMemoryDetail(MemoryItem memory) {
    Navigator.push(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (context, _, __) => _MemoryDetailPage(memory: memory),
...
class _MemoryDetailPageState extends State<_MemoryDetailPage>
  ...
  void initState() {
    super.initState();
    _scrollController.addListener(_onScroll);
    _loadImageSizeAndStartAnimation();
  }

  Future<void> _loadImageSizeAndStartAnimation() async {
    // 先加载图片尺寸
  ...
  Widget build(BuildContext context) {
    ...
    final maxImageWidth = screenWidth * 0.6;

    // 图片缩放：最小缩放到80%
    double imageScale = 1.0;
    bool isMinScale = false;
    if (_imageSize != null && _scrollOffset > 0) {
      final scrollProgress = (_scrollOffset / 200).clamp(0.0, 1.0);
      final targetScale = 1.0 - scrollProgress * 0.2;
      imageScale = targetScale.clamp(0.8, 1.0);
      isMinScale = targetScale <= 0.8;
    }

    // 计算剩余区域高度（弹窗上方的区域）
    final sheetPixelHeight = screenHeight * _sheetHeight;
    final remainingHeight = screenHeight - sheetPixelHeight - safeAreaTop;

    // 确保图片不会进入状态栏区域
    final maxImageHeight = remainingHeight - 16; // 留出16px的边距
    double displayImageWidth;
    double displayImageHeight;

    if (_imageSize != null) {
      final aspectRatio = _imageSize!.width / _imageSize!.height;
      displayImageWidth = maxImageWidth * imageScale;
      displayImageHeight = displayImageWidth / aspectRatio;

      // 如果图片高度超过最大高度，按比例缩小
      if (displayImageHeight > maxImageHeight) {
        displayImageHeight = maxImageHeight;
        displayImageWidth = displayImageHeight * aspectRatio;
      }
    } else {
      // 默认使用正方形
      displayImageWidth = maxImageWidth;
      displayImageHeight = maxImageWidth.clamp(0.0, maxImageHeight);
    }

    // 图片在剩余区域居中显示
    // 当图片已经缩小到80%后，位置固定不变，让弹窗覆盖在图片上
    double imageTop;
    if (isMinScale) {
      // 使用初始位置（弹窗高度为0.4时的位置）
      final initialSheetHeight = screenHeight * 0.4;
      final initialRemainingHeight =
          screenHeight - initialSheetHeight - safeAreaTop;
      imageTop =
          safeAreaTop +
          8 +
          (initialRemainingHeight - 16 - displayImageHeight) / 2;
    } else {
      imageTop =
          safeAreaTop + 8 + (remainingHeight - 16 - displayImageHeight) / 2;
    }
                                        ...
                                        Padding(
                                          ...
                                          child: Row(
                                            children: [
                                              ...
                                              const Spacer(),
                                              Text(
                                                '${widget.memory.createdAt.year}年${widget.memory.createdAt.month}月${widget.memory.createdAt.day}日',
  ...
  List<Widget> _buildDetailInfoForDetailPage() {
    final List<Widget> widgets = [];
